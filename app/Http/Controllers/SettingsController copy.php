...
class SettingsController extends Controller
{
    ...
    public function update_homepage_image(Request $request)
    {
        // hero image 
        if ($request->image) {
            $imageName = $this->imageUpload($request->image);
            DB::table('settings')->where('id', 97)->update(['option_value' => $imageName]);
        }
        // hero image 
        if ($request->sectionImage) {

            $imageName = $this->imageUpload($request->sectionImage);

            DB::table('settings')->where('id', 99)->update(['option_value' => $imageName]);
        }
        // header logo
        if ($request->logo) {
            $imageName = $this->imageUpload($request->logo);
            DB::table('settings')->where('id', 98)->update(['option_value' => $imageName]);
        }
    ...
    public function faqs()
    {
        ...
        // return view('setup.faqs.index', compact('faqs'));
        return view('setup.faqs.index');
    }
    ...
    public function faqDatatable(Request $request)
    {
        ...
        return Datatables::eloquent($faqs)->editColumn('question', function ($service) {
            return $service->question;
        })
            ->addColumn('answer', function ($service) {
                return $service->answer;
            })
            ->addColumn('page', function ($service) {
                return $service->page;
            })
            ->addColumn('action', function ($service) {

                return '<a class="" href="' . route('settings_edit_faqs', $service->id) . '"><i class="fas fa-edit"></i></a><a class="delete-item" href="' . route('settings_delete_faqs', $service->id) . '"><i class="fas fa-times text-danger ml-2"></i></a>';
            })
            ->rawColumns([
                'question',
                'answer',
    ...
    public function createFaqs()
    {
        ...
        return view('setup.faqs.create', compact('pages'));
    }
    ...
    public function editFaqs(Faq $faq)
    {
        $pages = $this->pageService->pages();
        // dd($pages);
        return view('setup.faqs.edit', compact('faq', 'pages'));
    }
    ...
    public function updateFaqs(Request $request, Faq $faq)
    {
        ...
        return redirect()->route('settings_show_faqs');
    }
    ...
    {
        return view('setup.messages.index');
    }
    public function messagesDatatable(Request $request)
    {
        $messages = Message::with(['user:id,first_name,last_name', 'receiver:id,first_name,last_name'])->orderBy('created_at', 'ASC');

        return Datatables::eloquent($messages)->addColumn('message_html', function ($message) {
            return view('setup.messages.message_list', compact('message'))->render();
        })
            ->rawColumns([
                'message_html'
            ])
        ...
        //     })
        //     ->addColumn('writer', function ($message) {
        //         return $message->receiver->first_name . " " . $message->receiver->last_name;
        //     })
