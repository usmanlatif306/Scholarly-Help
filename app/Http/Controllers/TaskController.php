...
class TaskController extends Controller
{
    ...
    public function submit_work(Request $request, Order $order, ApiOrderService $service)
    {
        ...
        // calling api for api order
        if ($order->api_user_id && $order->api_order_id) {
            $data['order_id'] = $order->api_order_id;
            $data['status'] = ORDER_STATUS_SUBMITTED_FOR_APPROVAL;
            $data['message'] = $submittedWork->message;
            $data['name'] = $submittedWork->name;
            $data['display_name'] = $submittedWork->display_name;
            $data['api_work_id'] = $submittedWork->id;
            $service->changeOrderStatus($data);
        }

        return redirect()->back()->withSuccess('Thank you. Your work has been submitted to client for approval');
    }

    public function start_working(Request $request, Order $order, ApiOrderService $service)
    {
        if (auth()->user()->id != $order->staff_id) {
            abort(404);
        }
        ...
        if ($order->api_user_id && $order->api_order_id) {
            $data['order_id'] = $order->api_order_id;
            $data['status'] = ORDER_STATUS_IN_PROGRESS;
            $service->changeOrderStatus($data);
        }
