...
class Order extends Model
{
    ...
    protected $fillable = [
        ...
        'update_via_sms',
        'billed',
        'guarantee', 'sub_category', 'subject', 'invoiced', 'api_order_id', 'api_user_id'
    ];
    ...
    static function task_dropdown()
    {
        ...
        $data['dead_line_list'] = [
            ...
            'today' => 'Today',
            'tommorrow' => 'Tomorrow',
            'day_after_tommorrow' => 'The day after tomorrow'
        ];
    ...
    static function dropdown(string $type = 'order')
    {
        // $data['service_cat'] = Service::with(['subCategories'])->orderBy('name', 'ASC')->whereNull('inactive')->get();

        $services = Service::with(['subCategories'])->orderBy('name', 'ASC')->whereNull('inactive')->get();
        $service_arr = [];
        ...
        $data['guarantee_list'] = Guarantee::orderBy('id', 'ASC')->whereNull('inactive')->get();
        ...
        if ($urgencies->count() > 0) {
            foreach ($urgencies as $urgency) {
                ...
                // $time =  ($urgency->type == 'hours') ? now()->addHours($urgency->value) : null;
