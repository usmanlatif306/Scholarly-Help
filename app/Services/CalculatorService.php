...
class CalculatorService
{
    ...
    */
    ...
    {
        $service = $request['sub_category'] ? SubCategory::find($request['sub_category']) : Service::find($request['service_id']);
        ...
        // When Price Type is fixed
        if ($service->price_type_id == PriceType::PerSlide) {
            $base_price = $service->price;
            $unit_name = PriceType::PerSlidePriceUnit;
        }
        ...
        $guarantee_price = $this->calculatePercentage($base_price, $guarantee->percentage_to_add);

        // Calculate Unit Price
        $unit_price = ($base_price + $work_level_price + $urgency_price + $guarantee_price);
