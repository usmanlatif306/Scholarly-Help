...
class OrderService
{
    function create(array $data)
    {
        ...
        $dates = explode(' ', $data['dead_line']);
        $date = $dates[0];
        $time = $date . ' ' . $data['time'];
        $data['dead_line'] = date("Y-m-d H:i:s", strtotime($time));
    ...
    private function record_attachments(Order $order, $data)
    {
        ...
        if (isset($data['files_data']) && is_array($data['files_data']) && count($data['files_data']) > 0) {
            $attachment = new Attachment();
            $attachment->name = $data['files_data']['name'];
            $attachment->display_name = $data['files_data']['display_name'];
            $attachment->user_id = $data['customer_id'];
            $order->attachments()->save($attachment);
        }
    }
