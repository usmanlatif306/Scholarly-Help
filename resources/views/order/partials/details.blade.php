<div class="row">
         ...
         <div class="row order-overview">
            ...
            <div class="col-md-6">
               ...
               <ol class="pl-4">
                  ...
                  $url = env('API_MAIN_URL').'attachments/download?file='.$attachment->name;
                  }else{
                  $url = route('download_attachment', 'file=' . $attachment->name);
                  }
                  @endphp
                  <li><a target="_blank" href="{{ $url }}">{{
                        $attachment->display_name }}</a></li>
                  @endforeach
               </ol>
            </div>
         </div>
   ...
   <div class="col-md-4">
      @if($order->customer_id == auth()->user()->id && $order->staff_id == NULL)
      <a class="btn btn-sm btn-warning mb-4" href="{{ route('choose_writer', $order->id) }}">
         <i class="fas fa-hand-pointer"></i> Choose Tutor</a>
      @elseif($order->customer_id == auth()->user()->id && $order->order_status_id == ORDER_STATUS_PENDING_PAYMENT)
      ...
      @endrole
      @if(auth()->user()->hasRole('staff') && settings('enable_browsing_work') == 'yes' && $order->order_status_id ==
      ORDER_STATUS_NEW && empty($order->staff_id))
