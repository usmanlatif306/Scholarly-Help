...
<submit-work :submit_work_url="'{{ route('submit_work', $order->id) }}'"
	:upload_attachment_url="'{{ route('order_upload_attachment') }}'"></submit-work>

@elseif($order->order_status_id == in_array($order->order_status_id, [ORDER_STATUS_NEW,
...
<form action="{{ route('start_working', $order->id) }}" method="POST" autocomplete="off">
	...
	<button class="btn btn-success btn-lg btn-block" type="submit" name="submit"><i class="fas fa-rocket"></i> Start
		Working</button>
