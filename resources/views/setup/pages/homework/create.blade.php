@extends('setup.index')
@section('title', 'Homework Sub Category Pages')
@section('setting_page')
@include('setup.partials.action_toolbar', [
'title' => (isset($page->id)) ? 'Edit Page' : 'Create new Page',
'hide_save_button' => TRUE,
'back_link' => ['title' => 'back to pages', 'url' => route("homework.pages.index")],
])
<form role="form" class="form-horizontal"
    action="{{ (isset($page->id)) ? route( 'homework.pages.update', $page->id) : route('homework.pages.store') }}"
    method="post" autocomplete="off">
    {{ csrf_field() }}
    @if(isset($page->id))
    {{ method_field('PATCH') }}
    @endif
    @include('setup.partials.action_toolbar', ['title' => 'Homework Sub Category Pages'])
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item active">Website Frontend</li>
            <li class="breadcrumb-item" aria-current="page">Homework Sub Category Pages</li>
        </ol>
    </nav>
    <div class="container">
        <div class="row">
