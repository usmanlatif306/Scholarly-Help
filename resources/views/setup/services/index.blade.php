...
<div class="actions-toolbar py-2 mb-1">
    <div class="row">
        ...
        <div class="col-md-6 text-right">
            <a href="{{route('service.create')}}" class="btn btn-sm btn-success">Create New</a>
        </div>
    </div>
    <hr>
</div>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="width: 10%;">
                                <a href="{{route('service.edit',$item->id)}}"><i class="fas fa-edit"></i></a>
                                <span style="cursor: pointer;font-size: 18px;margin-left: 5px;"
                                    onclick="return window.confirm(document.getElementById('service-{{$item->id}}').submit());"><i
                                        class="fas fa-times text-danger"></i></span>
                                <form id="service-{{$item->id}}" action="{{route('service.destroy',$item->id)}}"
                                    method="post">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                                </form>
