<div class="container" id="order">
    <div class="order-content">
        <home :services="{{ json_encode($data['service_id_list']) }}"
            :levels="{{ json_encode($data['work_level_id_list']) }}"
            :urgencies="{{ json_encode($data['urgency_id_list']) }}"
            :guarantees="{{ json_encode($data['guarantee_list']) }}"
            :spacings="{{ json_encode($data['spacings_list']) }}"
            :user_id="{{ (!Auth::guest()) ? auth()->user()->id : 'false' }}"
            :add_to_cart_url="'{{route('add_to_cart')}}'" :restricted_order_page_url="'{{ route('order_page') }}'"
            ...
            :email_marketing_url="'{{ route('email_marketing') }}'" :subjects="{{json_encode($data['subjects'])}}">
        </home>
    </div>
</div>